...
function ArrivalPage() {
  ...
  const filteredSections = useMemo(() => {
    if (!searchTerm.trim()) return Object.keys(sectionContent);
    const term = searchTerm.toLowerCase();
    return Object.keys(sectionContent).filter(id => {
  ...
  const equipmentSections = filteredSections.filter(id => ["aircon", "bakersOven", "brattPan", "chairs", "chiller", "combiOvens", "deepFryer", "defibrillator", "dishWasher", "dishes", "freezers", "fridges", "gas", "grillTops", "hangiCookers", "hotBoxes", "hotWater", "microwave"].includes(id));
  const cleaningSections = filteredSections.filter(id => ["cleanDining", "cleanKitchen", "cleanToilets", "cleanWharenui", "cleanEquipment"].includes(id));
  const facilitiesSections = filteredSections.filter(id => ["evacuation", "firstAid", "flag", "floors", "linen", "loadingDock", "mattressRoom", "lights", "parking", "pigBins", "recyclingBins", "rubbish", "showers", "smoking", "tables", "toilets", "trolleys", "vacuum", "wifi"].includes(id));
      ...
      {filteredSections.length === 0 ? (
        ...
        <div>
          ...
          {equipmentSections.length > 0 && (
            <CollapsibleGroupHeader groupId="equipment" title="Equipment & Appliances">
              {equipmentSections.map(id => (
          ...
          {/* Cleaning Section */}
          {cleaningSections.length > 0 && (
            <CollapsibleGroupHeader groupId="cleaning" title="Cleaning Instructions">
              {cleaningSections.map(id => (
                <CollapsibleSection key={id} id={id} title={sectionContent[id].title} color={sectionContent[id].color}>
                  {sectionContent[id].content()}
                </CollapsibleSection>
              ))}
          ...
          {facilitiesSections.length > 0 && (
            <CollapsibleGroupHeader groupId="facilities" title="Facilities & General Information">
              {facilitiesSections.map(id => (
                <CollapsibleSection key={id} id={id} title={sectionContent[id].title} color={sectionContent[id].color}>
                  {sectionContent[id].content()}
                </CollapsibleSection>
              ))}
