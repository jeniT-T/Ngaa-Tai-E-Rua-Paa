...
// --- Fallback content -------------------------------------------------
// Shown ONLY when the CMS has no content_items placed on "arrival" yet --
// e.g. right after a fresh install/fork, before an admin has added real
// guide content. IMPORTANT: this is shipped in the frontend JS bundle and
// is publicly visible to anyone (view source), so it must never contain
// real operational details (real WiFi passwords, real phone numbers, real
// equipment locations) for whichever marae is running the site. Keep this
// generic -- the real, marae-specific content belongs in the database only,
// entered/edited from the admin Content Manager.
const FALLBACK_ITEMS = [
  {
    id: "example-equipment",
    title: "Example: Equipment",
    category: "equipment",
    body: "No arrival guide content has been added yet. An admin can add real equipment instructions (ovens, chillers, dishwashers, etc.) from the Content Manager -- this placeholder will disappear once real items are added.",
  },
  {
    id: "example-cleaning",
    title: "Example: Cleaning Instructions",
    category: "cleaning",
    body: "No cleaning instructions have been added yet. An admin can add final-clean checklists for each space from the Content Manager.",
  },
  {
    id: "example-facilities",
    title: "Example: Facilities & General Information",
    category: "facilities",
    body: "No facilities information has been added yet. An admin can add details like parking, WiFi, toilets and first aid from the Content Manager.",
  },
];
  ...
  const items = useMemo(() => {
    ...
    return sections.map((item) => {
      ...
      return {
        id: String(item.id),
        title: item.title,
        color: item.color || GROUP_COLORS[category] || "#2c3e50",
        category,
        body: item.body,
    ...
    <div style={{ padding: "40px", maxWidth: "1200px", margin: "0 auto", textAlign: "left", fontFamily: "sans-serif" }}>
      ...
      <div style={{ padding: "20px", background: "#fafafa", borderRadius: "6px", border: "1px solid #ddd", textAlign: "center" }}>
        <p style={{ color: "#555", margin: 0, fontSize: "0.95rem" }}>
          For questions or issues, please contact your marae committee. (Admins: add your own
          contact details here, or via the arrival page heading in the Content Manager.)
        </p>
      </div>
    </div>
  );
